...
//var this.data.edata.ip="http://192.168.1.113/"
...
Page({
  ...
  */
  ...
  getData() {
    ...
    wx.request({
      url: 'http://47.97.158.11:8803/equipment/find?id=' + this.data.id,
      method: 'GET',
      header: {
  ...
  upImgs: function (imgurl) {
    ...
    wx.uploadFile({
      ...
      name: 'image',
      //图片要上传的后端接口地点
      url: 'http://47.97.158.11:8803/upload/uploadPic',
      //顺带的文本信息
      formData: {},
      success: (result) => {
        console.log(result);
  ...
  */
  ...
  onUnload: function () {
    ...
    wx.request({
      ...
      data: JSON.stringify(this.data.edata),
      method: 'POST',
      header: {
        'content-type': 'application/json'
      },
  ...
  btnsave() {
    ...
    wx.request({
      url: 'http://47.97.158.11:8803/equipment/create',
      data: JSON.stringify(that.data.edata),
      method: 'POST',
      header: {
        'content-type': 'application/json'
      },
      success: (res) => {
        wx.request({
          url: 'http://' + that.data.edata.ip + '/' + 'EPDw_', //仅为示例，并非真实的接口地址
          method: 'POST',
          header: {
            'content-type': 'application/json'
          },
  ...
  u_send(cmd, next) {
    ...
    wx.request({
      url: 'http://' + that.data.edata.ip + '/' + cmd, //仅为示例，并非真实的接口地址
      method: 'POST',
      header: {
        'content-type': 'application/json'
      },
      success: function (res) {
        ...
        that.setData({
          ...
          tips: "上传进度 " + (that.floor(((that.data.count + 1) / 96) * 100, 2)>100? 100:that.floor(((that.data.count + 1) / 96) * 100, 2) ) + "%"
        ...
        if (that.data.stInd == 0) return that.u_data(a, 0, 0, 100);
        else if (that.data.stInd == 1) return that.u_done();
        else{
         

          let temp = that.data.edata;
          temp.state = 0;
          temp.picture = that.data.uurl
          that.setData({
              edata: temp
          })
